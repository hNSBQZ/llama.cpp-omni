...
static void awq_marlin_repack_launch(
    const uint32_t * b_q_weight_ptr, uint32_t * out_ptr, int size_k,
    int size_n, int num_bits, bool is_a_8bit, int blocks, int max_shared_mem,
    cudaStream_t stream) {
  if (false) {
  }
  CALL_IF(4, false)
...
extern "C" bool ggml_cuda_marlin_awq_repack(
    const uint32_t* b_q_weight_ptr, uint32_t* out_ptr, int64_t size_k,
    int64_t size_n, int64_t num_bits, bool is_a_8bit, int device,
    void* stream_ptr) {
  ...
  awq_marlin_repack_launch(b_q_weight_ptr, out_ptr, int(size_k), int(size_n),
                           int(num_bits), is_a_8bit, blocks, max_shared_mem,
                           stream);
