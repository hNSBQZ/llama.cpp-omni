...
static void gptq_marlin_repack_launch(
    const uint32_t * b_q_weight_ptr, const uint32_t * perm_ptr,
    uint32_t * out_ptr, int size_k, int size_n, int num_bits, bool has_perm,
    bool is_a_8bit, int blocks, int max_shared_mem, cudaStream_t stream) {
  if (false) {
  }
...
extern "C" bool ggml_cuda_marlin_gptq_repack(
    const uint32_t* b_q_weight_ptr, const uint32_t* perm_ptr,
    uint32_t* out_ptr, int64_t size_k, int64_t size_n, int64_t num_bits,
    bool has_perm, bool is_a_8bit, int device, void* stream_ptr) {
  TORCH_CHECK(size_k % marlin::tile_k_size == 0);
  TORCH_CHECK(size_n % marlin::tile_n_size == 0);
  TORCH_CHECK(num_bits == 4 || num_bits == 8);

  int blocks = 0;
  CUDA_CHECK(cudaDeviceGetAttribute(&blocks, cudaDevAttrMultiProcessorCount, device));
  ...
  gptq_marlin_repack_launch(
      b_q_weight_ptr, perm_ptr, out_ptr, int(size_k), int(size_n),
      int(num_bits), has_perm, is_a_8bit, blocks, max_shared_mem, stream);
