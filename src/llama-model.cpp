...
static void llama_hparams_set_quant_checkpoint_format(llama_hparams & hparams, const std::string & value) {
    if (value.size() >= hparams.quant_checkpoint_format.size()) {
        throw std::runtime_error(
            format("%s: quantization checkpoint_format is too long (%zu >= %zu)", __func__, value.size(),
                   hparams.quant_checkpoint_format.size()));
    }

    std::memcpy(hparams.quant_checkpoint_format.data(), value.c_str(), value.size() + 1);
}
...
static bool llama_awq_marlin_debug_match_name(const char * name) {
    ...
    if (std::strstr(name, ".qweight") != nullptr || std::strstr(name, ".qzeros") != nullptr ||
...
static void llama_awq_marlin_debug_dump_tensor(const char * stage, const ggml_tensor * tensor, int64_t limit = 8) {
    ...
    oss << "awq-marlin-debug: stage=" << stage << " name=" << (name ? name : "<unnamed>")
        << " type=" << ggml_type_name(tensor->type) << " shape=" << llama_format_tensor_shape(tensor) << " nb=["
        ...
        case GGML_TYPE_F32:
            {
                ...
                oss << " finite_min=" << min_value << " finite_max=" << max_value << " nan=" << nan_count
                    << " inf=" << inf_count;
            }
            break;
        case GGML_TYPE_F16:
...
static int llama_cuda_device_from_tensor(const ggml_tensor * tensor) {
    ...
    if (dev_name == nullptr) {
        throw std::runtime_error(
            format("%s: unable to resolve CUDA device for tensor '%s'", __func__, ggml_get_name(tensor)));
    }
    ...
    if (std::strncmp(dev_name, GGML_CUDA_NAME, 4) != 0 || parsed.ec != std::errc() || parsed.ptr == dev_name + 4) {
        throw std::runtime_error(
            format("%s: tensor '%s' is not on a CUDA device (got '%s')", __func__, ggml_get_name(tensor), dev_name));
    }
...
static int llama_cuda_device_from_dev(ggml_backend_dev_t dev, const char * tensor_name) {
    ...
    if (dev_name == nullptr) {
        throw std::runtime_error(
            format("%s: unable to resolve CUDA device name for tensor '%s'", __func__, tensor_name));
    }
    ...
    if (std::strncmp(dev_name, GGML_CUDA_NAME, 4) != 0 || parsed.ec != std::errc() || parsed.ptr == dev_name + 4) {
        throw std::runtime_error(
            format("%s: unsupported CUDA device '%s' for tensor '%s'", __func__, dev_name, tensor_name));
    }
...
static ggml_backend_t llama_init_cuda_backend_for_tensor(const ggml_tensor * tensor, int * device_out) {
    ...
    if (dev != nullptr && buft == ggml_backend_dev_buffer_type(dev)) {
        ...
        if (cuda_dev == nullptr) {
            throw std::runtime_error(
                format("%s: no CUDA device available for tensor '%s'", __func__, ggml_get_name(tensor)));
        }
    ...
    if (backend == nullptr) {
        throw std::runtime_error(
            format("%s: failed to initialize CUDA backend for tensor '%s'", __func__, ggml_get_name(tensor)));
    }
...
static ggml_backend_buffer_ptr llama_alloc_cuda_tensor_buffer(ggml_backend_t backend, const ggml_tensor * tensor) {
    ...
    ggml_backend_buffer_ptr buf{ ggml_backend_buft_alloc_buffer(ggml_backend_get_default_buffer_type(backend),
                                                                ggml_nbytes(tensor)) };
    if (!buf) {
        throw std::runtime_error(
            format("%s: failed to allocate CUDA buffer for tensor '%s'", __func__, ggml_get_name(tensor)));
    }

    return buf;
}

static void llama_rehome_tensor_to_cuda(ggml_tensor *                          tensor,
...
static void llama_restore_awq_tensor_layout_to_hf_2d(ggml_tensor *                          tensor,
                                                     ggml_backend_t                         backend,
                                                     std::vector<ggml_backend_buffer_ptr> & owned_buffers) {
    GGML_ASSERT(tensor != nullptr);
    GGML_ASSERT(backend != nullptr);
    ...
    ggml_tensor             dst = *tensor;
    ggml_backend_buffer_ptr dst_buf{ ggml_backend_buft_alloc_buffer(ggml_backend_get_default_buffer_type(backend),
                                                                    ggml_nbytes(&dst)) };
    if (!dst_buf) {
        throw std::runtime_error(
            format("%s: failed to allocate buffer for tensor '%s'", __func__, ggml_get_name(tensor)));
    }
    dst.buffer = dst_buf.get();
    dst.data   = ggml_backend_buffer_get_base(dst_buf.get());
    ...
    if (tensor->type == GGML_TYPE_I32) {
        ...
        throw std::runtime_error(format("%s: unsupported tensor type '%s' for tensor '%s'", __func__,
                                        ggml_type_name(tensor->type), ggml_get_name(tensor)));
    }
...
static void llama_permute_awq_scales_to_cuda(ggml_tensor *                          scales,
                                             int64_t                                size_k,
                                             int64_t                                group_size,
                                             ggml_backend_t                         backend,
                                             std::vector<ggml_backend_buffer_ptr> & owned_buffers) {
    ...
    const int64_t perm_block    = use_full_perm ? (int64_t) k_scale_perm.size() : (int64_t) k_scale_perm_single.size();
    ...
    if (!dst_buf) {
        throw std::runtime_error(
            format("%s: failed to allocate CUDA buffer for tensor '%s'", __func__, ggml_get_name(scales)));
    }
...
static void llama_convert_awq_qzeros_to_marlin_cuda(ggml_tensor *                          qzeros,
                                                    int64_t                                num_bits,
                                                    int64_t                                size_n,
                                                    ggml_backend_t                         backend,
                                                    std::vector<ggml_backend_buffer_ptr> & owned_buffers) {
    ...
    if (!dst_buf) {
        throw std::runtime_error(
            format("%s: failed to allocate CUDA buffer for tensor '%s'", __func__, ggml_get_name(qzeros)));
    }
...
static void llama_repack_awq_qweight_for_marlin(ggml_tensor *                          qweight,
    ...
    const bool ok =
        ggml_cuda_marlin_awq_repack(static_cast<const uint32_t *>(src.data), static_cast<uint32_t *>(dst_data),
                                    qweight->ne[0], scales->ne[1], num_bits, false, device, nullptr);
    if (!ok) {
        throw std::runtime_error(
            format("%s: ggml_cuda_marlin_awq_repack failed for tensor '%s'", __func__, ggml_get_name(qweight)));
    }
...
static void llama_repack_gptq_qweight_for_marlin(ggml_tensor *                          qweight,
                                                 const ggml_tensor *                    scales,
                                                 bool                                   recenter_to_u8b128,
                                                 int64_t                                num_bits,
                                                 std::vector<ggml_backend_buffer_ptr> & owned_buffers) {
    GGML_ASSERT(qweight != nullptr);
    GGML_ASSERT(scales != nullptr);
    ...
    const int64_t pack_factor   = 32 / num_bits;
    const int64_t size_n        = scales->ne[1];
    const int64_t packed_size_k = qweight->ne[1];
    const int64_t size_k        = packed_size_k * pack_factor;

    GGML_ASSERT(pack_factor > 0);
    GGML_ASSERT(qweight->ne[0] == size_n);
    GGML_ASSERT(size_n % pack_factor == 0);
    ...
    ggml_tensor             src = *qweight;
    ggml_backend_buffer_ptr src_buf;
    std::vector<uint32_t>   host_src(ggml_nelements(qweight));

    ggml_backend_tensor_get(qweight, host_src.data(), 0, ggml_nbytes(qweight));

    // GPTQ qweight keeps the original HF buffer order for Marlin repack.
    std::vector<uint32_t> host_adjusted = std::move(host_src);

    if (recenter_to_u8b128) {
        uint8_t * bytes = reinterpret_cast<uint8_t *>(host_adjusted.data());
        for (size_t i = 0, n = host_adjusted.size() * sizeof(uint32_t); i < n; ++i) {
            // Convert signed int8 bytes to Marlin's U8B128 convention.
            bytes[i] ^= 0x80u;
        }
    }
    ...
    src.buffer = src_buf.get();
    src.data   = ggml_backend_buffer_get_base(src_buf.get());
    ggml_backend_tensor_set(&src, host_adjusted.data(), 0, ggml_nbytes(&src));

    ggml_backend_buffer_ptr dst_buf  = llama_alloc_cuda_tensor_buffer(backend.get(), qweight);
    void *                  dst_data = ggml_backend_buffer_get_base(dst_buf.get());

    const bool ok = ggml_cuda_marlin_gptq_repack(static_cast<const uint32_t *>(src.data), nullptr,
                                                 static_cast<uint32_t *>(dst_data), size_k, size_n, num_bits, false,
                                                 false, device, nullptr);
    if (!ok) {
        throw std::runtime_error(
            format("%s: ggml_cuda_marlin_gptq_repack failed for tensor '%s'", __func__, ggml_get_name(qweight)));
    }

    // Publish the new buffer only after the CUDA repack completes.
    ggml_backend_synchronize(backend.get());

    qweight->buffer = dst_buf.get();
    qweight->data   = dst_data;
    qweight->ne[0]  = size_k;
    qweight->ne[1]  = size_n / pack_factor;
    qweight->ne[2]  = 1;
    qweight->ne[3]  = 1;
    qweight->nb[0]  = ggml_type_size(qweight->type);
    qweight->nb[1]  = qweight->nb[0] * qweight->ne[0];
    qweight->nb[2]  = qweight->nb[1] * qweight->ne[1];
    qweight->nb[3]  = qweight->nb[2];
    owned_buffers.emplace_back(std::move(dst_buf));
}

static void llama_validate_gptq_qzeros_symmetric(const ggml_tensor * qzeros) {
    GGML_ASSERT(qzeros != nullptr);
    GGML_ASSERT(qzeros->type == GGML_TYPE_I32);

    std::vector<int32_t> host(ggml_nelements(qzeros));
    ggml_backend_tensor_get(qzeros, host.data(), 0, ggml_nbytes(qzeros));

    static constexpr int32_t k_expected = 0x7f7f7f7f;
    for (size_t i = 0; i < host.size(); ++i) {
        if (host[i] != k_expected) {
            throw std::runtime_error(
                format("%s: tensor '%s' is not the expected symmetric GPTQ qzeros layout at element %zu", __func__,
                       ggml_get_name(qzeros), i));
        }
    }
}

static void llama_validate_gptq_g_idx_sequential(const ggml_tensor * g_idx, int64_t group_size) {
    GGML_ASSERT(g_idx != nullptr);
    GGML_ASSERT(g_idx->type == GGML_TYPE_I32);
    GGML_ASSERT(group_size > 0);

    std::vector<int32_t> host(ggml_nelements(g_idx));
    ggml_backend_tensor_get(g_idx, host.data(), 0, ggml_nbytes(g_idx));

    for (size_t i = 0; i < host.size(); ++i) {
        const int32_t expected = int32_t(i / size_t(group_size));
        if (host[i] != expected) {
            throw std::runtime_error(format(
                "%s: tensor '%s' is not the expected sequential GPTQ g_idx layout at element %zu (got %d expected %d)",
                __func__, ggml_get_name(g_idx), i, host[i], expected));
        }
    }
}

static void llama_repack_qwen3_awq_tensors_to_marlin(llama_model &                          model,
                                                     std::vector<ggml_backend_buffer_ptr> & owned_buffers) {
    ...
    auto repack_triplet = [&](ggml_tensor * qweight, ggml_tensor * qzeros, ggml_tensor * scales) {
        ...
        // Offline-converted Marlin GGUF already stores AWQ triplets in Marlin
        // layout. In this case we only ensure the tensors are resident on CUDA
        // and skip the runtime transforms.
        ...
        // AWQ triplets were exported with .T to satisfy gguf/ggml shape
        // semantics. Restore the original HF layout before applying the Marlin
        // transforms.
        ...
        llama_convert_awq_qzeros_to_marlin_cuda(qzeros, model.hparams.quant_bits, scales->ne[1], backend.get(),
                                                owned_buffers);
        llama_permute_awq_scales_to_cuda(scales, qweight->ne[0], model.hparams.quant_group_size, backend.get(),
                                         owned_buffers);
    };
...
static void llama_repack_qwen3_gptq_tensors_to_marlin(llama_model &                          model,
                                                      std::vector<ggml_backend_buffer_ptr> & owned_buffers) {
    if (model.arch != LLM_ARCH_QWEN3 || model.hparams.quant_method != LLAMA_QUANTIZATION_METHOD_GPTQ) {
        return;
    }

    if (model.hparams.quant_bits != 8) {
        throw std::runtime_error(format("%s: only GPTQ 8-bit Marlin repack is supported", __func__));
    }
    if (model.hparams.quant_group_size != 128) {
        throw std::runtime_error(format("%s: only GPTQ group_size=128 is supported", __func__));
    }
    if (model.hparams.quant_checkpoint_format[0] != '\0' &&
        std::strcmp(model.hparams.quant_checkpoint_format.data(), "gptq") != 0) {
        throw std::runtime_error(format("%s: unsupported GPTQ checkpoint_format='%s' for this path", __func__,
                                        model.hparams.quant_checkpoint_format.data()));
    }
    if (model.hparams.quant_desc_act) {
        throw std::runtime_error(format("%s: GPTQ desc_act=true is not supported for this path", __func__));
    }
    auto repack_quartet = [&](ggml_tensor * qweight, ggml_tensor * qzeros, ggml_tensor * scales, ggml_tensor * g_idx) {
        if (qweight == nullptr && qzeros == nullptr && scales == nullptr && g_idx == nullptr) {
            return;
        }
        if (qweight == nullptr || qzeros == nullptr || scales == nullptr || g_idx == nullptr) {
            throw std::runtime_error(format("%s: incomplete GPTQ tensor quartet for '%s'", __func__,
                                            qweight ? ggml_get_name(qweight) : "<missing-qweight>"));
        }

        llama_validate_gptq_qzeros_symmetric(qzeros);
        llama_validate_gptq_g_idx_sequential(g_idx, model.hparams.quant_group_size);

        int              device = 0;
        ggml_backend_ptr backend{ llama_init_cuda_backend_for_tensor(qweight, &device) };

        // Restore GPTQ scales to the HF layout.
        llama_restore_awq_tensor_layout_to_hf_2d(scales, backend.get(), owned_buffers);

        // MiniCPM-o GPTQ-W8 qweight does not need recentering.
        llama_repack_gptq_qweight_for_marlin(qweight, scales, false, model.hparams.quant_bits, owned_buffers);
        llama_permute_awq_scales_to_cuda(scales, qweight->ne[0], model.hparams.quant_group_size, backend.get(),
                                         owned_buffers);
    };

    for (llama_layer & layer : model.layers) {
        repack_quartet(layer.wq_gptq_qweight, layer.wq_gptq_qzeros, layer.wq_gptq_scale, layer.wq_gptq_g_idx);
        repack_quartet(layer.wk_gptq_qweight, layer.wk_gptq_qzeros, layer.wk_gptq_scale, layer.wk_gptq_g_idx);
        repack_quartet(layer.wv_gptq_qweight, layer.wv_gptq_qzeros, layer.wv_gptq_scale, layer.wv_gptq_g_idx);
        repack_quartet(layer.wo_gptq_qweight, layer.wo_gptq_qzeros, layer.wo_gptq_scale, layer.wo_gptq_g_idx);
        repack_quartet(layer.ffn_gate_gptq_qweight, layer.ffn_gate_gptq_qzeros, layer.ffn_gate_gptq_scale,
                       layer.ffn_gate_gptq_g_idx);
        repack_quartet(layer.ffn_up_gptq_qweight, layer.ffn_up_gptq_qzeros, layer.ffn_up_gptq_scale,
                       layer.ffn_up_gptq_g_idx);
        repack_quartet(layer.ffn_down_gptq_qweight, layer.ffn_down_gptq_qzeros, layer.ffn_down_gptq_scale,
                       layer.ffn_down_gptq_g_idx);
    }
}
#else
static void llama_repack_qwen3_awq_tensors_to_marlin(llama_model &                          model,
...
static void llama_repack_qwen3_gptq_tensors_to_marlin(llama_model &                          model,
    ...
    GGML_UNUSED(owned_buffers);
    if (model.arch == LLM_ARCH_QWEN3 && model.hparams.quant_method == LLAMA_QUANTIZATION_METHOD_GPTQ) {
        throw std::runtime_error("Qwen3 GPTQ Marlin repack requires GGML_USE_CUDA");
    }
}
#endif
        ...
        case LLM_ARCH_QWEN3:
            {
                ...
                ml.get_key(LLM_KV_QUANTIZATION_DAMP_PERCENT, hparams.quant_damp_percent, false);
                ml.get_key(LLM_KV_QUANTIZATION_DESC_ACT, hparams.quant_desc_act, false);
                ml.get_key(LLM_KV_QUANTIZATION_STATIC_GROUPS, hparams.quant_static_groups, false);
                ml.get_key(LLM_KV_QUANTIZATION_SYM, hparams.quant_sym, false);
                std::string quant_checkpoint_format;
                ml.get_key(LLM_KV_QUANTIZATION_TRUE_SEQUENTIAL, hparams.quant_true_sequential, false);
                ml.get_key(LLM_KV_QUANTIZATION_CHECKPOINT_FORMAT, quant_checkpoint_format, false);
                llama_hparams_set_quant_checkpoint_format(hparams, quant_checkpoint_format);
...
bool llama_model::load_tensors(llama_model_loader & ml) {
            ...
            case LLM_ARCH_GRANITE_MOE:
                {
                    ...
                    const char * tts_tensor_names[] = { "emb_code.0.weight",
                                                        "emb_text.weight",
                    ...
                    for (const char * tts_name : tts_tensor_names) {
                        ...
                        if (tts_tensor) {
                            ...
                            LLAMA_LOG_INFO(
                                "%s: skipping TTS tensor %s (size = %zu bytes) -- will be loaded separately\n",
            ...
            case LLM_ARCH_QWEN3:
                {
                    const bool     is_awq_qwen3       = hparams.quant_method == LLAMA_QUANTIZATION_METHOD_AWQ &&
                                                        hparams.quant_bits > 0 && hparams.quant_group_size > 0;
                    const bool     is_gptq_qwen3      = hparams.quant_method == LLAMA_QUANTIZATION_METHOD_GPTQ &&
                                                        hparams.quant_bits > 0 && hparams.quant_group_size > 0;
                    const uint32_t awq_pack_factor    = is_awq_qwen3 ? 32u / hparams.quant_bits : 0u;
                    const uint32_t gptq_pack_factor   = is_gptq_qwen3 ? 32u / hparams.quant_bits : 0u;
                    const int      quant_weight_flags = (is_awq_qwen3 || is_gptq_qwen3) ? TENSOR_NOT_REQUIRED : 0;
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        auto create_awq_tensor_triplet = [&](ggml_tensor *& qweight, ggml_tensor *& qzeros,
                                                             ggml_tensor *& scales, llm_tensor qweight_id,
                                                             llm_tensor qzeros_id, llm_tensor scales_id,
                            ...
                            qweight = create_tensor(tn(qweight_id, i), { in_features, packed_out_features },
                                                    TENSOR_NOT_REQUIRED);
                            qzeros =
                                create_tensor(tn(qzeros_id, i), { n_groups, packed_out_features }, TENSOR_NOT_REQUIRED);
                            scales = create_tensor(tn(scales_id, i), { n_groups, out_features }, TENSOR_NOT_REQUIRED);
                        };

                        auto create_gptq_tensor_quartet = [&](ggml_tensor *& qweight, ggml_tensor *& qzeros,
                                                              ggml_tensor *& scales, ggml_tensor *& g_idx,
                                                              llm_tensor qweight_id, llm_tensor qzeros_id,
                                                              llm_tensor scales_id, llm_tensor g_idx_id,
                                                              int64_t in_features, int64_t out_features) {
                            if (!is_gptq_qwen3) {
                                return;
                            }

                            GGML_ASSERT(gptq_pack_factor > 0);
                            GGML_ASSERT(in_features % hparams.quant_group_size == 0);
                            GGML_ASSERT(out_features % gptq_pack_factor == 0);

                            const int64_t n_groups            = in_features / hparams.quant_group_size;
                            const int64_t packed_in_features  = in_features / gptq_pack_factor;
                            const int64_t packed_out_features = out_features / gptq_pack_factor;

                            // Keep the exported GPTQ qweight shape for loader validation.
                            qweight = create_tensor(tn(qweight_id, i), { out_features, packed_in_features },
                                                    TENSOR_NOT_REQUIRED);
                            qzeros =
                                create_tensor(tn(qzeros_id, i), { n_groups, packed_out_features }, TENSOR_NOT_REQUIRED);
                            scales = create_tensor(tn(scales_id, i), { n_groups, out_features }, TENSOR_NOT_REQUIRED);
                            g_idx  = create_tensor(tn(g_idx_id, i), { in_features }, TENSOR_NOT_REQUIRED);
                        };
                        ...
                        layer.wv = create_tensor(tn(LLM_TENSOR_ATTN_V, "weight", i), { n_embd, n_embd_gqa },
                                                 quant_weight_flags);
                        layer.wo = create_tensor(tn(LLM_TENSOR_ATTN_OUT, "weight", i),
                                                 { n_embd_head_k * n_head, n_embd }, quant_weight_flags);

                        create_awq_tensor_triplet(layer.wq_awq_qweight, layer.wq_awq_qzeros, layer.wq_awq_scale,
                                                  LLM_TENSOR_ATTN_Q_AWQ_QWEIGHT, LLM_TENSOR_ATTN_Q_AWQ_QZEROS,
                                                  LLM_TENSOR_ATTN_Q_AWQ_SCALES, n_embd, n_embd_head_k * n_head);
                        create_awq_tensor_triplet(layer.wk_awq_qweight, layer.wk_awq_qzeros, layer.wk_awq_scale,
                                                  LLM_TENSOR_ATTN_K_AWQ_QWEIGHT, LLM_TENSOR_ATTN_K_AWQ_QZEROS,
                                                  LLM_TENSOR_ATTN_K_AWQ_SCALES, n_embd, n_embd_gqa);
                        create_awq_tensor_triplet(layer.wv_awq_qweight, layer.wv_awq_qzeros, layer.wv_awq_scale,
                                                  LLM_TENSOR_ATTN_V_AWQ_QWEIGHT, LLM_TENSOR_ATTN_V_AWQ_QZEROS,
                                                  LLM_TENSOR_ATTN_V_AWQ_SCALES, n_embd, n_embd_gqa);
                        create_awq_tensor_triplet(layer.wo_awq_qweight, layer.wo_awq_qzeros, layer.wo_awq_scale,
                                                  LLM_TENSOR_ATTN_OUT_AWQ_QWEIGHT, LLM_TENSOR_ATTN_OUT_AWQ_QZEROS,
                                                  LLM_TENSOR_ATTN_OUT_AWQ_SCALES, n_embd_head_k * n_head, n_embd);

                        create_gptq_tensor_quartet(layer.wq_gptq_qweight, layer.wq_gptq_qzeros, layer.wq_gptq_scale,
                                                   layer.wq_gptq_g_idx, LLM_TENSOR_ATTN_Q_GPTQ_QWEIGHT,
                                                   LLM_TENSOR_ATTN_Q_GPTQ_QZEROS, LLM_TENSOR_ATTN_Q_GPTQ_SCALES,
                                                   LLM_TENSOR_ATTN_Q_GPTQ_G_IDX, n_embd, n_embd_head_k * n_head);
                        create_gptq_tensor_quartet(layer.wk_gptq_qweight, layer.wk_gptq_qzeros, layer.wk_gptq_scale,
                                                   layer.wk_gptq_g_idx, LLM_TENSOR_ATTN_K_GPTQ_QWEIGHT,
                                                   LLM_TENSOR_ATTN_K_GPTQ_QZEROS, LLM_TENSOR_ATTN_K_GPTQ_SCALES,
                                                   LLM_TENSOR_ATTN_K_GPTQ_G_IDX, n_embd, n_embd_gqa);
                        create_gptq_tensor_quartet(layer.wv_gptq_qweight, layer.wv_gptq_qzeros, layer.wv_gptq_scale,
                                                   layer.wv_gptq_g_idx, LLM_TENSOR_ATTN_V_GPTQ_QWEIGHT,
                                                   LLM_TENSOR_ATTN_V_GPTQ_QZEROS, LLM_TENSOR_ATTN_V_GPTQ_SCALES,
                                                   LLM_TENSOR_ATTN_V_GPTQ_G_IDX, n_embd, n_embd_gqa);
                        create_gptq_tensor_quartet(layer.wo_gptq_qweight, layer.wo_gptq_qzeros, layer.wo_gptq_scale,
                                                   layer.wo_gptq_g_idx, LLM_TENSOR_ATTN_OUT_GPTQ_QWEIGHT,
                                                   LLM_TENSOR_ATTN_OUT_GPTQ_QZEROS, LLM_TENSOR_ATTN_OUT_GPTQ_SCALES,
                                                   LLM_TENSOR_ATTN_OUT_GPTQ_G_IDX, n_embd_head_k * n_head, n_embd);
                        ...
                        layer.ffn_norm = create_tensor(tn(LLM_TENSOR_FFN_NORM, "weight", i), { n_embd }, 0);
                        layer.ffn_gate =
                            create_tensor(tn(LLM_TENSOR_FFN_GATE, "weight", i), { n_embd, n_ff }, quant_weight_flags);
                        layer.ffn_down =
                            create_tensor(tn(LLM_TENSOR_FFN_DOWN, "weight", i), { n_ff, n_embd }, quant_weight_flags);
                        layer.ffn_up =
                            create_tensor(tn(LLM_TENSOR_FFN_UP, "weight", i), { n_embd, n_ff }, quant_weight_flags);

                        create_awq_tensor_triplet(layer.ffn_gate_awq_qweight, layer.ffn_gate_awq_qzeros,
                                                  layer.ffn_gate_awq_scale, LLM_TENSOR_FFN_GATE_AWQ_QWEIGHT,
                                                  LLM_TENSOR_FFN_GATE_AWQ_QZEROS, LLM_TENSOR_FFN_GATE_AWQ_SCALES,
                                                  n_embd, n_ff);
                        create_awq_tensor_triplet(layer.ffn_down_awq_qweight, layer.ffn_down_awq_qzeros,
                                                  layer.ffn_down_awq_scale, LLM_TENSOR_FFN_DOWN_AWQ_QWEIGHT,
                                                  LLM_TENSOR_FFN_DOWN_AWQ_QZEROS, LLM_TENSOR_FFN_DOWN_AWQ_SCALES, n_ff,
                                                  n_embd);
                        create_awq_tensor_triplet(layer.ffn_up_awq_qweight, layer.ffn_up_awq_qzeros,
                                                  layer.ffn_up_awq_scale, LLM_TENSOR_FFN_UP_AWQ_QWEIGHT,
                                                  LLM_TENSOR_FFN_UP_AWQ_QZEROS, LLM_TENSOR_FFN_UP_AWQ_SCALES, n_embd,
                                                  n_ff);

                        create_gptq_tensor_quartet(layer.ffn_gate_gptq_qweight, layer.ffn_gate_gptq_qzeros,
                                                   layer.ffn_gate_gptq_scale, layer.ffn_gate_gptq_g_idx,
                                                   LLM_TENSOR_FFN_GATE_GPTQ_QWEIGHT, LLM_TENSOR_FFN_GATE_GPTQ_QZEROS,
                                                   LLM_TENSOR_FFN_GATE_GPTQ_SCALES, LLM_TENSOR_FFN_GATE_GPTQ_G_IDX,
                                                   n_embd, n_ff);
                        create_gptq_tensor_quartet(layer.ffn_down_gptq_qweight, layer.ffn_down_gptq_qzeros,
                                                   layer.ffn_down_gptq_scale, layer.ffn_down_gptq_g_idx,
                                                   LLM_TENSOR_FFN_DOWN_GPTQ_QWEIGHT, LLM_TENSOR_FFN_DOWN_GPTQ_QZEROS,
                                                   LLM_TENSOR_FFN_DOWN_GPTQ_SCALES, LLM_TENSOR_FFN_DOWN_GPTQ_G_IDX,
                                                   n_ff, n_embd);
                        create_gptq_tensor_quartet(
                            layer.ffn_up_gptq_qweight, layer.ffn_up_gptq_qzeros, layer.ffn_up_gptq_scale,
                            layer.ffn_up_gptq_g_idx, LLM_TENSOR_FFN_UP_GPTQ_QWEIGHT, LLM_TENSOR_FFN_UP_GPTQ_QZEROS,
                            LLM_TENSOR_FFN_UP_GPTQ_SCALES, LLM_TENSOR_FFN_UP_GPTQ_G_IDX, n_embd, n_ff);
                    }
                }
                break;
            case LLM_ARCH_QWEN3MOE:
    ...
    llama_repack_qwen3_gptq_tensors_to_marlin(*this, pimpl->bufs);
    ...
    llm_build_falcon(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // self-attention
            {
                ...
                ggml_tensor * Kcur =
                    ggml_view_3d(ctx0, cur, n_embd_head, n_head_kv, n_tokens, n_embd_head * sizeof(float), cur->nb[1],
        ...
        for (int il = 0; il < n_layer; ++il) {
...

            {
                ...
                if (model.layers[il].wqkv) {
                    ...
                    Qcur = ggml_view_3d(ctx0, cur, n_embd_head, n_head, n_tokens, n_embd_head * sizeof(float),
                                        cur->nb[1], 0 * sizeof(float) * (n_embd));
                    Kcur = ggml_view_3d(ctx0, cur, n_embd_head, n_head_kv, n_tokens, n_embd_head * sizeof(float),
                                        cur->nb[1], 1 * sizeof(float) * (n_embd));
                    Vcur = ggml_view_3d(ctx0, cur, n_embd_head, n_head_kv, n_tokens, n_embd_head * sizeof(float),
...
struct llm_build_dream : public llm_graph_context {
    llm_build_dream(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        //copied from qwen2
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_llada : public llm_graph_context {
    llm_build_llada(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
...
struct llm_build_qwen3 : public llm_graph_context {
    llm_build_qwen3(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        const auto has_gptq_linear = [&](ggml_tensor * qweight, ggml_tensor * qzeros, ggml_tensor * scales,
                                         ggml_tensor * g_idx) {
            return is_gptq_qwen3 && qweight != nullptr && qzeros != nullptr && scales != nullptr && g_idx != nullptr;
        };

        const auto build_qwen3_linear = [&](ggml_tensor * dense, ggml_tensor * awq_qweight, ggml_tensor * awq_qzeros,
                                            ggml_tensor * awq_scales, ggml_tensor * gptq_qweight,
                                            ggml_tensor * gptq_qzeros, ggml_tensor * gptq_scales,
                                            ggml_tensor * gptq_g_idx, ggml_tensor * input, int il) -> ggml_tensor * {
            if (has_awq_linear(awq_qweight, awq_qzeros, awq_scales)) {
                return build_awq_marlin_mm(input, awq_qweight, awq_qzeros, awq_scales, il);
            }

            if (has_gptq_linear(gptq_qweight, gptq_qzeros, gptq_scales, gptq_g_idx)) {
                return build_gptq_marlin_mm(input, gptq_qweight, gptq_scales, il);
            }
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // self-attention
            {
                ...
                ggml_tensor * Qcur = build_qwen3_linear(
                    model.layers[il].wq, model.layers[il].wq_awq_qweight, model.layers[il].wq_awq_qzeros,
                    model.layers[il].wq_awq_scale, model.layers[il].wq_gptq_qweight, model.layers[il].wq_gptq_qzeros,
                    model.layers[il].wq_gptq_scale, model.layers[il].wq_gptq_g_idx, cur, il);
                cb(Qcur, "Qcur", il);

                ggml_tensor * Kcur = build_qwen3_linear(
                    model.layers[il].wk, model.layers[il].wk_awq_qweight, model.layers[il].wk_awq_qzeros,
                    model.layers[il].wk_awq_scale, model.layers[il].wk_gptq_qweight, model.layers[il].wk_gptq_qzeros,
                    model.layers[il].wk_gptq_scale, model.layers[il].wk_gptq_g_idx, cur, il);
                cb(Kcur, "Kcur", il);

                ggml_tensor * Vcur = build_qwen3_linear(
                    model.layers[il].wv, model.layers[il].wv_awq_qweight, model.layers[il].wv_awq_qzeros,
                    model.layers[il].wv_awq_scale, model.layers[il].wv_gptq_qweight, model.layers[il].wv_gptq_qzeros,
                    model.layers[il].wv_gptq_scale, model.layers[il].wv_gptq_g_idx, cur, il);
                cb(Vcur, "Vcur", il);
                ...
                cur = build_qwen3_linear(model.layers[il].wo, model.layers[il].wo_awq_qweight,
                                         model.layers[il].wo_awq_qzeros, model.layers[il].wo_awq_scale,
                                         model.layers[il].wo_gptq_qweight, model.layers[il].wo_gptq_qzeros,
                                         model.layers[il].wo_gptq_scale, model.layers[il].wo_gptq_g_idx, cur, il);
            ...
            ggml_tensor * ffn_up =
                build_qwen3_linear(model.layers[il].ffn_up, model.layers[il].ffn_up_awq_qweight,
                                   model.layers[il].ffn_up_awq_qzeros, model.layers[il].ffn_up_awq_scale,
                                   model.layers[il].ffn_up_gptq_qweight, model.layers[il].ffn_up_gptq_qzeros,
                                   model.layers[il].ffn_up_gptq_scale, model.layers[il].ffn_up_gptq_g_idx, cur, il);
            cb(ffn_up, "ffn_up", il);

            ggml_tensor * ffn_gate =
                build_qwen3_linear(model.layers[il].ffn_gate, model.layers[il].ffn_gate_awq_qweight,
                                   model.layers[il].ffn_gate_awq_qzeros, model.layers[il].ffn_gate_awq_scale,
                                   model.layers[il].ffn_gate_gptq_qweight, model.layers[il].ffn_gate_gptq_qzeros,
                                   model.layers[il].ffn_gate_gptq_scale, model.layers[il].ffn_gate_gptq_g_idx, cur, il);
            ...
            cur =
                build_qwen3_linear(model.layers[il].ffn_down, model.layers[il].ffn_down_awq_qweight,
                                   model.layers[il].ffn_down_awq_qzeros, model.layers[il].ffn_down_awq_scale,
                                   model.layers[il].ffn_down_gptq_qweight, model.layers[il].ffn_down_gptq_qzeros,
                                   model.layers[il].ffn_down_gptq_scale, model.layers[il].ffn_down_gptq_g_idx, cur, il);
            cb(cur, "ffn_out", il);
    ...
    llm_build_gemma(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // self-attention
            {
                ...
                cur = build_attn(inp_attn, model.layers[il].wo, NULL, Qcur, Kcur, Vcur, nullptr, nullptr, nullptr, 1.0f,
                                 il);
            }
...
struct llm_build_gemma3n_iswa : public llm_graph_context {
    ...
    llm_build_gemma3n_iswa(const llama_model & model, const llm_graph_params & params) :
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            ggml_tensor * corrected =
                altup_correct(predictions, attn_ffw_laurel_gated, il);     // [n_embd, n_tokens, n_altup]
            ...
            ggml_tensor * first_prediction;                                // [n_embd, n_tokens]
            {
                ...
                first_prediction =
                    build_lora_mm(model.layers[il].per_layer_proj, first_prediction);  // [n_embd, n_tokens]
                first_prediction =
            ...
            // equivalent to python code: corrected_predictions[1:] += first_prediction
            {
                ...
                ggml_tensor * slice_rest =
                    ggml_view_3d(ctx0, corrected, n_embd, n_tokens, n_altup - 1, ggml_row_size(corrected->type, n_embd),
    ...
    ggml_tensor * calc_magnitude(ggml_tensor * x) { return ggml_sqrt(ctx0, ggml_sum_rows(ctx0, ggml_sqr(ctx0, x))); }
    ...
    ggml_tensor * gaussian_topk(ggml_tensor * x) {
        ...
        ggml_tensor * std =
            ggml_sqrt(ctx0, ggml_scale(ctx0, ggml_sum_rows(ctx0, ggml_sqr(ctx0, ggml_sub(ctx0, x, mean))),
                                       1.0f / (float) (x->ne[0] - 1)));
    ...
    ggml_tensor * altup_correct(ggml_tensor * predictions, ggml_tensor * activated, int il) {
        ...
        ggml_tensor * all_coefs =
            build_lora_mm(model.layers[il].altup_correct_coef, modalities);  // [n_altup, n_tokens]
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // self_attention
            {
                ...
                if (is_swa) {
                    ...
                    Qcur = ggml_rope_ext(ctx0, Qcur, inp_pos, nullptr, n_rot, rope_type, n_ctx_orig, freq_base, 1.0,
                                         0.0, 1.0, beta_fast, beta_slow);

                    Kcur = ggml_rope_ext(ctx0, Kcur, inp_pos, nullptr, n_rot, rope_type, n_ctx_orig, freq_base, 1.0,
                                         0.0, 1.0, beta_fast, beta_slow);
                } else {
                    Qcur = ggml_rope_ext(ctx0, Qcur, inp_pos, nullptr, n_rot, rope_type, n_ctx_orig, freq_base,
                                         freq_scale, ext_factor, attn_factor, beta_fast, beta_slow);
    ...
    llm_build_t5_enc(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // self-attention
            {
                ...
                cur = build_attn(inp_attn, model.layers[il].wo_enc, nullptr, Qcur, Kcur, Vcur, kq_b, nullptr, nullptr,
                                 1.0f, il);
                cb(cur, "kqv_out", il);
            }
    ...
    llm_build_t5_dec(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        for (int il = 0; il < dec_n_layer; ++il) {
            ...
            // self-attention
            {
                ...
                cur = build_attn(inp_attn_self, model.layers[il].wo, model.layers[il].bo, Qcur, Kcur, Vcur, kq_b,
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // self-attention
            {
                ...
                ggml_tensor * Kcur = ggml_view_3d(ctx0, cur, n_embd_head, n_head_kv, n_tokens,
                                                  n_embd_head * sizeof(float), cur->nb[1], 1 * cur->nb[0] * (n_embd));
                ggml_tensor * Vcur =
                    ggml_view_3d(ctx0, cur, n_embd_head, n_head_kv, n_tokens, n_embd_head * sizeof(float), cur->nb[1],
...
struct llm_build_granite : public llm_graph_context {
    llm_build_granite(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            if (!is_moe_layer) {
                ...
                ggml_tensor * moe_out = build_moe_ffn(
                    cur, model.layers[il].ffn_gate_inp, model.layers[il].ffn_up_exps, model.layers[il].ffn_gate_exps,
                    model.layers[il].ffn_down_exps, model.layers[il].ffn_exp_probs_b, n_expert, n_expert_used,
                    LLM_FFN_SILU, true, false, 0.0, LLAMA_EXPERT_GATING_FUNC_TYPE_SOFTMAX, il);
                cb(moe_out, "ffn_moe_out", il);
...
struct llm_build_falcon_h1 : public llm_graph_context_mamba {
    llm_build_falcon_h1(const llama_model & model, const llm_graph_params & params) : llm_graph_context_mamba(params) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            Qcur = ggml_rope_ext(ctx0, Qcur, inp_pos, nullptr, n_rot, hparams.rope_type, n_ctx_orig, freq_base,
                                 freq_scale, ext_factor, attn_factor, beta_fast, beta_slow);

            Kcur = ggml_rope_ext(ctx0, Kcur, inp_pos, nullptr, n_rot, hparams.rope_type, n_ctx_orig, freq_base,
                                 freq_scale, ext_factor, attn_factor, beta_fast, beta_slow);
  ...
  private:
    ggml_tensor * build_plamo2_attn_layer(llm_graph_input_attn_kv * inp,
        ...
        // self-attention
        {
            ...
            ggml_tensor * Kcur =
                ggml_view_3d(ctx0, qkv, n_embd_head_k, n_head_kv, n_tokens, n_embd_head_k * sizeof(float), qkv->nb[1],
                             k_offset * ggml_element_size(qkv));
            ggml_tensor * Vcur =
                ggml_view_3d(ctx0, qkv, n_embd_head_v, n_head_kv, n_tokens, n_embd_head_v * sizeof(float), qkv->nb[1],
    ...
    ggml_tensor * build_plamo2_mamba_layer(llm_graph_input_rs * inp,
        ...
        ggml_tensor * x = ggml_view_4d(ctx0, zx, head_dim, n_heads, n_seq_tokens, n_seqs, zx->nb[1], zx->nb[2],
                                       zx->nb[3], head_dim * ggml_element_size(zx));
        ...
        // SSM
        {
            ...
            const int64_t dt_dim = std::max(64, int(hparams.n_embd / 16));
            ggml_tensor * B =
                ggml_view_3d(ctx0, x_bcdt, d_state, n_seq_tokens, n_seqs, x_bcdt->nb[1], x_bcdt->nb[2], 0);
            ...
            ggml_build_forward_expand(
                gf, ggml_cpy(ctx0,
                             ...
                             ggml_view_1d(
                                 ctx0, ssm_states_all, n_heads * head_dim * d_state * n_seqs,
    ...
    llm_build_hunyuan_moe(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            ggml_tensor * cur_moe = build_moe_ffn(cur, model.layers[il].ffn_gate_inp, model.layers[il].ffn_up_exps,
                                                  model.layers[il].ffn_gate_exps, model.layers[il].ffn_down_exps,
                                                  nullptr, n_expert, n_expert_used, LLM_FFN_SILU,
                                                  true,  // norm_topk_prob
...
struct llm_build_openai_moe_iswa : public llm_graph_context {
    llm_build_openai_moe_iswa(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // MoE branch
            cur = build_moe_ffn(
                cur, model.layers[il].ffn_gate_inp, model.layers[il].ffn_gate_inp_b, model.layers[il].ffn_up_exps,
                model.layers[il].ffn_up_exps_b, model.layers[il].ffn_gate_exps, model.layers[il].ffn_gate_exps_b,
                model.layers[il].ffn_down_exps, model.layers[il].ffn_down_exps_b, nullptr, n_expert, n_expert_used,
                LLM_FFN_SWIGLU_OAI_MOE, false, false, 0.0, LLAMA_EXPERT_GATING_FUNC_TYPE_SOFTMAX_WEIGHT, il);
            cb(cur, "ffn_moe_out", il);
    ...
    llm_build_smallthinker(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            ggml_tensor * ffn_out = build_moe_ffn(
                cur, nullptr, model.layers[il].ffn_up_exps, model.layers[il].ffn_gate_exps,
                model.layers[il].ffn_down_exps, nullptr, n_expert, n_expert_used, LLM_FFN_RELU, true, false, 0.0,
                ...
                if (llm_arch_is_recurrent(arch)) {
                    ...
                    if (hparams.swa_type != LLAMA_SWA_TYPE_NONE) {
                        ...
                        res = new llama_kv_cache_iswa(*this, params.type_k, params.type_v, !cparams.flash_attn,
                                                      ...
                                                      n_ctx_per_stream, cparams.n_seq_max, cparams.n_ubatch, padding,
                                                      nullptr, reuse);
                    } else {
                        GGML_ASSERT(!hparams.is_swa_any());

                        res = new llama_kv_cache(*this, params.type_k, params.type_v, !cparams.flash_attn,
                                                 cparams.offload_kqv, cparams.kv_unified, n_ctx_per_stream,
                                                 cparams.n_seq_max, padding, hparams.n_swa, hparams.swa_type, nullptr,
                                                 nullptr);
                    }
                }
            }
