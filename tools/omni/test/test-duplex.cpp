...
static void duplex_test_case(struct omni_context * ctx_omni,
    ...
    for (int il = 0; il < cnt; ++il) {
        ...
        omni_perf_mark(ctx_omni, "api.chunk_prefill", "start", il, -1.0, prefill_detail.c_str());
        ...
        omni_perf_mark(ctx_omni, "api.chunk_prefill", "end", il, prefill_dt * 1000.0, prefill_detail.c_str());
        ...
        omni_perf_mark(ctx_omni, "api.chunk_decode", "start", il, -1.0, nullptr);
        bool decode_ok = stream_decode(ctx_omni, "./");
        auto t3 = std::chrono::high_resolution_clock::now();
        double decode_dt = std::chrono::duration<double>(t3 - t2).count();
        omni_perf_mark(ctx_omni, "api.chunk_decode", "end", il, decode_dt * 1000.0, nullptr);
