...
bool LocDiTModel::validate_weights() const {
    ...
    if (!weights.in_proj_weight || !weights.in_proj_bias ||
        !weights.cond_proj_weight || !weights.cond_proj_bias ||
        !weights.out_proj_weight || !weights.out_proj_bias ||
        !weights.time_mlp_linear1_weight || !weights.time_mlp_linear1_bias ||
    ...
    if (weights.in_proj_bias->ne[0] != hidden_size ||
        weights.cond_proj_bias->ne[0] != hidden_size ||
...
bool LocDiTModel::init_manual(const VoxCPM2LocDiTConfig & cfg,
                              const VoxCPM2LocDiTWeights & manual_weights) {
    free();
...
ggml_tensor * LocDiTModel::sinusoidal_embedding(ggml_context * ctx,
                                                ggml_tensor * scalar,
                                                int dim,
                                                float scale) const {
...
ggml_tensor * LocDiTModel::build_time_embedding(ggml_context * ctx, ggml_tensor * t_scalar) const {
    ...
    return timestep_mlp(ctx,
                        sinusoidal,
                        weights.time_mlp_linear1_weight,
                        weights.time_mlp_linear1_bias,
                        weights.time_mlp_linear2_weight,
                        weights.time_mlp_linear2_bias);
}
...
ggml_tensor * LocDiTModel::build_delta_time_embedding(ggml_context * ctx, ggml_tensor * dt_scalar) const {
    ...
    return timestep_mlp(ctx,
                        sinusoidal,
                        weights.delta_time_mlp_linear1_weight,
                        weights.delta_time_mlp_linear1_bias,
                        weights.delta_time_mlp_linear2_weight,
                        weights.delta_time_mlp_linear2_bias);
}
...
ggml_tensor * LocDiTModel::build_time_token(ggml_context * ctx,
...
ggml_tensor * LocDiTModel::reshape_mu_tokens(ggml_context * ctx, ggml_tensor * mu) const {
    ...
    return ggml_reshape_2d(ctx,
                           mu,
                           config.transformer.hidden_size,
                           mu->ne[0] / config.transformer.hidden_size);
}
...
ggml_tensor * LocDiTModel::forward_projected(ggml_context * ctx,
    ...
    ggml_tensor * hidden = voxcpm2_transformer_forward(ctx,
                                                       ...
                                                       nullptr,
                                                       nullptr);
    ggml_tensor * hidden_out = ggml_view_2d(ctx,
                                            hidden,
                                            config.transformer.hidden_size,
                                            seq_len,
                                            hidden->nb[1],
...
void LocDiTModel::forward_cfg_pair_projected(ggml_context * ctx,
    ...
    if (total_len > config.transformer.max_length) {
        *conditioned = forward_projected(ctx,
                                         x_proj,
                                         conditioned_prefix,
                                         cond_proj,
                                         ...
                                         prefix_len,
                                         seq_len);
        *unconditioned = forward_projected(ctx,
                                           x_proj,
    ...
    ggml_tensor * paired_hidden = voxcpm2_transformer_forward(ctx,
                                                              config.transformer,
                                                              weights.transformer,
                                                              paired_seq,
                                                              positions,
    ...
    ggml_tensor * conditioned_hidden = ggml_view_2d(ctx,
                                                    paired_hidden,
                                                    config.transformer.hidden_size,
                                                    seq_len,
                                                    paired_hidden->nb[1],
                                                    static_cast<size_t>(prefix_len + generated_prefix_tokens) * paired_hidden->nb[1]);
    ggml_tensor * unconditioned_hidden = ggml_view_2d(ctx,
                                                      paired_hidden,
                                                      config.transformer.hidden_size,
                                                      seq_len,
                                                      paired_hidden->nb[1],
...
ggml_tensor * LocDiTModel::forward_cfg_pair(ggml_context * ctx,
    ...
    return ggml_add(ctx,
                    unconditioned,
                    ggml_scale(ctx, ggml_sub(ctx, conditioned, unconditioned), cfg_rate));
}
